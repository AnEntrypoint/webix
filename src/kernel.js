...
export class XOSKernel {
  ...
  async runX86_64Bytes(bytes, { argv = [], env, progname = "/program", rootfsTarBytes } = {}) {
    ...
    this.kernelActor.send({ type: "SPAWN", pid, process: proc });
    proc.send({ type: "START" });
    const result = await host.runElf(bytes, { argv, progname });
    proc.send({ type: "EXIT", code: result.exitCode });
    this.kernelActor.send({ type: "EXIT", pid });
    return { pid, exitCode: result.exitCode, stdout: result.stdout, stderr: result.stderr, signal: result.signal };
  }
  signal(pid, signal) {
